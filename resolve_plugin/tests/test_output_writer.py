...
These tests exercise the pure encoding functions directly, avoiding
imports of the full output_writer module (which transitively requires
torch/torchvision via backend.frame_io).  This allows the tests to
run on any machine without GPU dependencies.
"""
...
# ── Inline re-implementations of the encoding functions ──────────────────
...
class TestAtomicWriteBytes:
    ...
    def test_writes_file(self, tmp_path):
        ...
        _atomic_write_bytes(b"hello world", dest)
    ...
    def test_no_tmp_files_left(self, tmp_path):
        ...
        dest = str(tmp_path / "test.ppm")
        _atomic_write_bytes(b"data", dest)
    ...
    def test_overwrites_existing(self, tmp_path):
        ...
        dest = str(tmp_path / "test.ppm")
        _atomic_write_bytes(b"first", dest)
        _atomic_write_bytes(b"second", dest)
        with open(dest, "rb") as f:
